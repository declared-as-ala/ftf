...
- `MatchFinalizationService` currently catches discipline failures inside the transaction and continues; suspension serving, standings, round completion, and notifications then run best-effort after commit without a durable replay record.
- `MatchCorrectionService` currently rebuilds standings only. `SuspensionService` creates a ledger row and decrements outside one transaction. Canonical discipline queries are not consistently organization-scoped.
